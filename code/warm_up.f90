subroutine get_triangle()
	...
	integer k1,k2,k3,k4,k0,  tri(8*num_site,4)
        ...
        write(1212,*)'neib', j1, j2, ji, i

        if(i.gt.ns1.and.i.le.num_site)then !!!neibors, unclosed
        write(1213,*)'neib', j1, j2, ns1, i
        k0=k0+1
        ind2e(ji, ns1, 1)=k0
        tri3b(k0,ns1,1)=i
        ...
        write(222,*)j1,j2, ij+kk1
        endif
13      continue
        
        tri3s(k0, ns1, 3)= i
        write(1111,*)'tri3s', i, j2, j1, tri3s(k0, ns1, 1),k0
        endif
        endif
        enddo
        endif
        enddo
        enddo
        nleg2(ns1, 1)=k0  !!! for each system size ns1, there nleg2 of C_iCj pairs
        write(1213,*)ns1,k0,'nleg21'
        enddo
!!!! above is for bond operators
        ...
        if(kagome.eq.1)then
        write(1213,*)k0, ns1, 'neib11', j11, j22, i
        if(mod(ii,nleg).eq.nleg-1)then
        tri3s(k0, ns1,2)=-tri3s(k0, ns1, 2)
        write(1213,*)k0, ns1, 'neib11', j11, j22, i,'sign'
        endif
        ...
        write(222,*) tri3(j11,j22,ij+kk1), j11, j22, ij+kk1
        ...
        write(1213,*)ns1,k0,'nleg22'
        enddo

        write(*,*)'ring triangle', nleg2(1:4, 1)
        write(*,*)'ring triangle1', nleg2(1:4, 2)
...
!Read program paramters from the disk
...
!!!!!!!!!!!!!
        ...
        open(202)
        do i=1, num_site
        write(202,15)i,(neib(i,2*j1-1),j1=1,9)
15      format(10i7)
        enddo
        close(202)
...
subroutine Write_parameter
	...
	open(101,file="data.in")

	!For Model parameter: For hopping t
	write(101,"(A24)") "For_hopt_parameter(hopt)"
	write(101,"(A7,I4)") "number=",hopt_num
	write(101,"(A8,F8.4)") "minimum=",hopt_min
	write(101,"(A9,F8.4)") "interval=",hopt_int
	write(101,*)

	!For DMRG parameter: Up-spin electron number
	write(101,"(A22)") "For_NsUp_parameter(up)"
	write(101,"(A7,I4)") "number=",num_up_num
	write(101,"(A8,I3)") "minimum=",num_up_min
	write(101,"(A9,I2)") "interval=",num_up_int
	close(101)
...
subroutine Read_parameter
	...
	!For Model parameter
	read(101,"(A24)") char24
	read(101,"(A7,I4)") char7,hopt_num
	read(101,"(A8,F8.4)") char8,hopt_min
	read(101,"(A9,F8.4)") char9,hopt_int
	read(101,*)

	!For DMRG parameter: Up-spin electron number
	read(101,"(A22)") char22
	read(101,"(A7,I4)") char7,num_up_num
	read(101,"(A8,I3)") char8,num_up_min
	read(101,"(A9,I2)") char9,num_up_int
	close(101)
...
subroutine Get_model_name(sysmod,envmod)
	...
	!call str_link_str(str_name,str_len,num)
...
subroutine Get_site_operator_su2
	...
	!<1>: For st_elec_up: c^+_{up spin}
        ...
        ! for tjmodel, s=1,1state (U1-2states),  s=0, 1state for hole
        !! for hubmodel,  s=1,1state,  2 states for hole and double occupancy
	...
	idx=2
		...
		num_elec%sub(idx)%sdim=1
		allocate(num_elec%sub(idx)%mat(1,1))
		num_elec%sub(idx)%mat(1,1)=2.0d0   !! dsqrt(2J+1)=1
                        endif
	...
	!<7>: For st_si: %%Sz_{up spin}
		...
		st_si%sub(2)%mat(1,1)=dsqrt(2.0d0) !!!!1.d0 !!!!!!!(num_ups-num_downs)/2.0d0
                if(hubmodel==1) st_si%sub(3)%mat(1,1)=1.d0 !!!!!!!(num_ups-num_downs)/2.0d0
...
subroutine Get_site_operator
	...
	st_elec_up%sub(1)%sdim=1
	allocate(st_elec_up%sub(1)%mat(1,1))
	st_elec_up%sub(1)%mat(1,1)=1.0d0


	!<2>: For st_elec_down: c^+_{down spin}
	st_elec_down%len=1
	st_elec_down%num=1
	st_elec_down%dim=1
	st_elec_down%up_dif=0
	st_elec_down%down_dif=down_bias
	allocate(st_elec_down%sub(st_elec_down%num))

	!Matrix element <0,down|c^+_{down}|0,0>
	st_elec_down%sub(1)%num_up=0
	st_elec_down%sub(1)%num_down=0
	st_elec_down%sub(1)%row_dim=1
	st_elec_down%sub(1)%sdim=1
	allocate(st_elec_down%sub(1)%mat(1,1))
	st_elec_down%sub(1)%mat(1,1)=1.0d0


	!<3>: For num_elec_up: n_{up spin}
	num_elec_up%len=1
	num_elec_up%num=N_max
	num_elec_up%dim=N_max
	num_elec_up%up_dif=0
	num_elec_up%down_dif=0
	allocate(num_elec_up%sub(num_elec_up%num))

	idx=0
	do num_ups=0,1
	do num_downs=0,1
	if((num_ups*num_downs)/=1) then
		idx=idx+1
		num_elec_up%sub(idx)%num_up=num_ups
		num_elec_up%sub(idx)%num_down=num_downs
		num_elec_up%sub(idx)%row_dim=1
		num_elec_up%sub(idx)%sdim=1
		allocate(num_elec_up%sub(idx)%mat(1,1))
		num_elec_up%sub(idx)%mat(1,1)=num_ups*1.0d0
	endif
	end do
	end do


	!<4>: For num_down: n_{down spin}
	num_elec_down%len=1
	num_elec_down%num=N_max
	num_elec_down%dim=N_max
	num_elec_down%up_dif=0
	num_elec_down%down_dif=0
	allocate(num_elec_down%sub(num_elec_down%num))
	...
	if((num_ups*num_downs)/=1) then
		idx=idx+1
		num_elec_down%sub(idx)%num_up=num_ups
		num_elec_down%sub(idx)%num_down=num_downs
		num_elec_down%sub(idx)%row_dim=1
		num_elec_down%sub(idx)%sdim=1
		allocate(num_elec_down%sub(idx)%mat(1,1))
		num_elec_down%sub(idx)%mat(1,1)=num_downs*1.0d0
	endif
	end do
	end do


	!<5>: For n_i= n_{i,up spin}+n_{i,down spin}
	call block_pass_info(num_elec_up,num_elec)
	do i=1,N_max
		num_elec%sub(i)%mat=num_elec_up%sub(i)%mat+num_elec_down%sub(i)%mat
	end do
	...
	st_sd%num=1
	st_sd%dim=1
	st_sd%up_dif=up_bias
	st_sd%down_dif=-down_bias
	allocate(st_sd%sub(st_sd%num))

	!Matrix element <up+1,down-1|S^+|up,down>
	st_sd%sub(1)%num_up=0
	st_sd%sub(1)%num_down=1
	st_sd%sub(1)%row_dim=1
	st_sd%sub(1)%sdim=1
	allocate(st_sd%sub(1)%mat(1,1))
	st_sd%sub(1)%mat(1,1)=1.0d0
	...
	if((num_ups*num_downs)/=1) then
		idx=idx+1
		st_si%sub(idx)%num_up=num_ups
		st_si%sub(idx)%num_down=num_downs
		st_si%sub(idx)%row_dim=1
		st_si%sub(idx)%sdim=1
		allocate(st_si%sub(idx)%mat(1,1))
		st_si%sub(idx)%mat(1,1)=1.d0 !!!!!!!(num_ups-num_downs)/2.0d0
	endif
	end do
	end do
	...
	if((num_ups*num_downs)/=1) then
		idx=idx+1
		st_sz%sub(idx)%num_up=num_ups
		st_sz%sub(idx)%num_down=num_downs
		st_sz%sub(idx)%row_dim=1
		st_sz%sub(idx)%sdim=1
		allocate(st_sz%sub(idx)%mat(1,1))
		st_sz%sub(idx)%mat(1,1)=(num_ups-num_downs)/2.0d0
	endif
	end do
	end do


	!<8>: For st_sz_up: S^z_{up spin}
	st_sz_up%len=1
	st_sz_up%num=N_max
	st_sz_up%dim=N_max
	st_sz_up%up_dif=0
	st_sz_up%down_dif=0
	allocate(st_sz_up%sub(st_sz_up%num))
	...
	if((num_ups*num_downs)/=1) then
		idx=idx+1
		st_sz_up%sub(idx)%num_up=num_ups
		st_sz_up%sub(idx)%num_down=num_downs
		st_sz_up%sub(idx)%row_dim=1
		st_sz_up%sub(idx)%sdim=1
		allocate(st_sz_up%sub(idx)%mat(1,1))
		st_sz_up%sub(idx)%mat(1,1)=num_ups/2.0d0
	endif
	end do
	end do


	!<9>: For st_sz_down: S^z_{down spin}
	st_sz_down%len=1
	st_sz_down%num=N_max
	st_sz_down%dim=N_max
	st_sz_down%up_dif=0
	st_sz_down%down_dif=0
	allocate(st_sz_down%sub(st_sz_down%num))
	...
	if((num_ups*num_downs)/=1) then
		idx=idx+1
		st_sz_down%sub(idx)%num_up=num_ups
		st_sz_down%sub(idx)%num_down=num_downs
		st_sz_down%sub(idx)%row_dim=1
		st_sz_down%sub(idx)%sdim=1
		allocate(st_sz_down%sub(idx)%mat(1,1))
		st_sz_down%sub(idx)%mat(1,1)=-num_downs/2.0d0
	endif
	end do
	end do

end subroutine Get_site_operator
...
subroutine Get_hamiltonian_site(site_ham)
	...
	!<1>: Get single site Hamiltonian
		...
		call block_transfer(num_elec,site_ham%ham)
...
subroutine Get_single_site_basis
  ...
  st_basis%len=1
        ...
        do i=1, num_elec%num
                ...
                st_basis%sub(idx)%new_num_up=num_elec%sub(i)%num_up
                st_basis%sub(idx)%new_num_down=num_elec%sub(i)%num_down
...
subroutine warm_up_point(bg_id,end_id,trun_err,levels)
  ...
  if(hubbard(1).ne.0.0)then
          do i=1,sys%ham%num
                sys%ham%sub(i)%mat=hubbard(1)*st_double%sub(i)%mat
!!!old_block%sub(i)%mat
        end do
        endif
	...
	call Get_hamiltonian_site(env)
        ...
        if(esite(nposi_lat(3)).ne.0.0)then
          do i=1,env%ham%num
                env%ham%sub(i)%mat=esite(nposi_lat(3))*num_elec%sub(i)%mat !!!old_block%sub(i)%mat
        end do
        endif

 if(hubbard(nposi_lat(3)).ne.0.0)then
          do i=1,env%ham%num
             env%ham%sub(i)%mat=hubbard(nposi_lat(3))*st_double%sub(i)%mat
!!!old_block%sub(i)%mat
        end do
        endif
	...
	do while((label+1)<=end_id)
		...
		tot_down=int((tot_num_down*1.0d0/Num_site)*(sys_bs%len+env_bs%len)+0.5)
            ...
            !    if(tot_up==sys_bs%len+env_bs%len)then
             !   if(tot_num_up.ne.num_site)then
              !          tot_up=tot_up-2
               !                 endif
               !                 endif
        !! early allowing holes
