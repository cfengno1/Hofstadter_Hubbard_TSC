...
subroutine Get_density_matrix_sys_new(sys_den,wave,levels)
	...
	call deallocate_wave(eigvecs)
        ...
        do i=1,sys_den%num
                sys_den%sub(i)%mat=(0.1*sys_den%sub(i)%mat+1.9*tmp_den%sub(i)%mat)/2.0d0
        end do
...
subroutine Get_density_matrix_env_new(env_den,wave,levels)
	...
	call deallocate_wave(eigvecs)
        ...
        do i=1,env_den%num
                env_den%sub(i)%mat=(0.1*env_den%sub(i)%mat+1.9*tmp_den%sub(i)%mat)/2.0d0
        end do
        call deallocate_block(tmp_den)
                endif
...
subroutine Get_truncation_operator(den_mat,trun_op,trun_err,num_state)
	...
	do k=1,tmp%num
		...
		deallocate(rwork)
                ...
                deallocate(work,rwork)
                 ...
                 j=den_mat%len-Num_site/2
                if(abs(j).le.50) then
                write(1000,*)Num_site,"Nup=",tot_num_up,"Ndown=",tot_num_down,Num_State,den_mat%len,k
                do i=1,min(sub_dim,6)
                   write(1000,*) i,tmp%sub(k)%eigval(sub_dim-i+1),tmp%sub(k)%num_up,tmp%sub(k)%num_down
                end do
                write(1000,*)
                endif
	...
	if(system_block) then
		...
		sys_len=Num_site-den_mat%len
         ...
         if(sys_len==num_site/2)Entropy0=Entropys
	open(60,file="Entropy.dat",position='append')
	write(60,*) "N=",Num_site,"N_up=",tot_num_up,"N_down="&
				 &,tot_num_down,"Err=",tmp_err,"Sys_len=",sys_len,"S=",Entropys
	close(60)
110 format(A2,F6.3,1X,A2,F6.3,1X,A2,I3,1X,A5,I3,1X,A7,I3,1X,A4,E12.4,1X,A8,I3,1X,A2,F11.8)

	!Get Entanglement spectrum
	if(Spectrum) then
	if(den_mat%len==(Num_site/2)) then
		open(61,file="Entanglement.dat",position='append')
		write(61,*) "N=",Num_site,"N_up=",tot_num_up,"N_down="&
					 &,tot_num_down,"Num_State=",Num_State

		do i=1,min(tmp%dim,100)
			write(61,"(I4,F16.12)") i,eigvalue(i)
		end do
		write(61,*)
		close(61)
	endif
	...
	close(60)
112 format(A8,I3,2X,A10,I5,2X,A4,E12.4)
